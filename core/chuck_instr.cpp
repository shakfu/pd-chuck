...
Chuck_Object * instantiate_and_initialize_object( Chuck_Type * type, Chuck_VM_Shred * shred, Chuck_VM * vm )
{
    ...
    if( !type->ugen_info )
    {
        ...
        else if( isa( type, vm->env()->t_cclass ) ) object = new Chuck_Type( vm->env(), te_class, type->name, type, type->size );
        // TODO: is this ok?
                               ...
                               Chuck_Type * type )
{
    ...
    if( capacity >= top )
    {
        ...
        if( kind == kindof_INT ) // ISSUE: 64-bit (fixed 1.3.1.0)
        {
            ...
            // initialize object | 1.5.0.0 (ge) use array type instead of base t_carray
            // for the object->type_ref to contain more specific information
            initialize_object( baseX, type );
            // initialize_object( baseX, vm->env()->t_carray );
            return baseX;
        }
    ...
    // initialize_object( theBase, vm->env()->t_carray );
    return theBase;
