...
Chuck_Env::~Chuck_Env()
{
    ...
    // this should go last, as all the t_* types are of type t_cclass
    SAFE_UNLOCK_DELETE(t_cclass);
}
...
t_CKBOOL Chuck_Env::init()
{
    ...
    t_cclass = new Chuck_Type( this, te_class, "Type", t_object, sizeof(void *) );
...
t_CKBOOL type_engine_init_special( Chuck_Env * env, Chuck_Type * objT )
{
    ...
    initialize_object( objT, env->t_cclass );
    ...
    for( t_CKINT i = 0; i < funcs.size(); i++ )
    {
        ...
        while( f )
        {
            ...
            initialize_object( f->value_ref->type, env->t_cclass );
            // next f
            f = f->next;
        }
    }
...
Chuck_Env * type_engine_init( Chuck_Carrier * carrier )
{
    ...
    init_class_type( env, env->t_cclass ); // 1.5.0.0
    ...
    type_engine_init_special( env, env->t_carray );
...
t_CKBOOL type_engine_import_class_end( Chuck_Env * env )
{
    ...
    if( env->class_def->name != env->t_object->name && env->class_def->name != env->t_carray->name )
    {
        // initialize the type as object | 1.5.0.0 (ge) added
        initialize_object( env->class_def, env->t_cclass );
    }
...
Chuck_Type * Chuck_Context::new_Chuck_Type( Chuck_Env * env )
{
    ...
    // check if t_cclass has itself been initialized
    // this is only for types created before Type (t_cclass) initialized,
    ...
    if( env->t_cclass->info != NULL )
    {
        // initialize it as Type object | 1.5.0.0 (ge) added
        initialize_object( type, env->t_cclass );
    }
...
//       this function was created after adding the complexity of t_cclass being
...
t_CKBOOL type_engine_is_base_static( Chuck_Env * env, Chuck_Type * baseType )
{
    ...
    // check
    return isa( baseType, env->t_cclass ) && (baseType->actual_type != NULL);
}
